...
- **定期更新の実行手順**: `.claude/skills/update-blog/SKILL.md`（`/update-blog`）にまとまっています。4時間ごと（1日6回）のcronから呼び出されます。手動更新を頼まれたときもこのスキルを使ってください。記事を1件以上追加してpushしたら、末尾でDiscordに更新ログを通知します（`scripts/notify-discord.mjs`）
- **アクセス数レポート**: `.claude/skills/report-analytics/SKILL.md`（`/report-analytics`）。1日1回のcronから呼び出され、`scripts/report-analytics.mjs` がCloudflare Web Analyticsの直近24時間のアクセス数をDiscordに通知します
...
- `.env`（`STEAM_API_KEY`・`DISCORD_WEBHOOK_URL`・`CLOUDFLARE_API_TOKEN`・`CLOUDFLARE_ACCOUNT_ID`・`CF_ANALYTICS_SITE_TAG`）を含む秘密情報は絶対にコミットしない。`.claude/hooks/git-secret-guard.sh` がpush前に機械的にブロックするが、それに頼らず自分でも注意する
